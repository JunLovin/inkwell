...
import { Card, IconBox, Button, Loader } from "@/shared/components/ui";
import { NoteCard } from "@/shared/components/ui/note/NoteCard";
...
export default function DashboardPage() {
  ...
  const stats = useMemo(() => {
    if (!notes) return { total: 0, active: 0, favorite: 0, archived: 0 };
    return {
      total: notes.filter((n) => !n.isDeleted).length,
      active: notes.filter((n) => !n.isDeleted && !n.isArchived).length,
  ...
  useEffect(() => {
    if (!notes || animated.current) return;
        ...
        {recentNotes.length === 0 ? (
          <Card variant="ghost" padding="none">
            <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
              <IconBox icon={<FileText size={18} />} variant="default" size="lg" />
              <div>
